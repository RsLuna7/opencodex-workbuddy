...
describe("provider registry parity", () => {
  ...
  test("GUI preset projection preserves current featured set plus key catalog and custom", () => {
   ...
   expect(featured).toEqual([
     ...
     "mimo-free",
      // This fork's two additions. Both are first-class picker rows on a fork install, so the
      // pinned featured set has to name them or the GUI would silently drop them.
      "workbuddy", "zai-plan",
   ]);

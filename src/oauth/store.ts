...
function normalizeCredential(cred: unknown): OAuthCredentials | null {
  ...
  if (candidate.codebuddy && typeof candidate.codebuddy === "object") {
    ...
    const cleanCodebuddy = (value: unknown, max: number): string | undefined => {
      if (typeof value !== "string") return undefined;
      const trimmed = value.trim();
      return trimmed && trimmed.length <= max && !/[\x00-\x1f\x7f]/.test(trimmed) ? trimmed : undefined;
    };
  ...
  if (candidate.zaiPlan && typeof candidate.zaiPlan === "object") {
    ...
    const cleanZaiPlan = (value: unknown, max: number): string | undefined => {
      if (typeof value !== "string") return undefined;
      const trimmed = value.trim();
      return trimmed && trimmed.length <= max && !/[\x00-\x1f\x7f]/.test(trimmed) ? trimmed : undefined;
    };
    const deviceMid = cleanZaiPlan(raw.deviceMid, 128);
    const userId = cleanZaiPlan(raw.userId, 256);
    const sessionId = cleanZaiPlan(raw.sessionId, 128);
    const codingKey = cleanZaiPlan(raw.codingKey, 256);

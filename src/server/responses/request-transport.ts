...
export async function prepareResponsesTransport(
  ...
  /**
   * True when the current rotation deliberately did NOT persist the operator's active
   * account. WorkBuddy 6004 cools one account×model pair, so the next turn must not open
   * by replaying that cooled credential -- but the operator's own choice must survive.
   */
  ...
  const applyFailoverSnapshot = async (
    ...
    if (persistActive) {
      ...
      // Model-scoped rotation (WorkBuddy 6004) rebinds THIS request only. The credential is
      // re-read below rather than trusted from the caller's snapshot, so a removed or
      // reauth-flagged account still refuses the rotation instead of dispatching on it.
      const row = getAccountCredentialWithStatus(route.providerName, snapshot.accountId);
      if (!row || row.needsReauth) return null;
      oauthSelection = oauthSelection
        ? { accountId: snapshot.accountId, revision: oauthSelection.revision }
    ...
    if (snapshot.codebuddy || route.providerName === CODEBUDDY_PROVIDER_ID) {
      // CodeBuddy / WorkBuddy routes on account-scoped headers (uid / enterprise / domain),
      // exactly like Kiro's routing metadata: a token-only swap would pair B's bearer with
      // A's identity. Continue past the original request too, as the kiro branch above does.
  ...
  /**
   * One rotation DECISION for every site that can observe a credential refusal.
   *
   * Deliberately side-effect free: each site owns its own send-budget reservation, and the
   * four sites settle it differently (#4709 -- an adapter-owned ladder spends its own
   * reservation while a helper-routed replay hands the permit down). Fusing the send into
   * here would rotate a credential at a site whose budget then refused the replay, cooling
   * an account and moving the route for a send that never happens.
   *
   * What IS shared is the decision itself, because that is where the sites drifted: the
   * continuation loop never grew the arm the streaming loop did, so a 429 stayed terminal
   * there (#2568's follow-up).
   *
   * WorkBuddy adds a second shape on top of the 429: the vendor reports 6004/14018 in an
   * HTTP 402 body, and 402 is not a status Codex retries. Scope comes from that hint --
   * 6004 cools one account×model pair and deliberately does NOT persist the rotation, so
   * the operator's active account survives a single model going quiet.
   */
 const planGenericOAuthRotation = async (
   refusal: Response | { status: number; message?: string },
  ): Promise<{ failedAccountId: string; target: GenericRotateTarget; persistActive: boolean } | null> => {
    // A preflight adapter error has a status and a message but no Response to read a body
    // from, so the durable envelope is parsed from that message instead. The provider gate
    // lives here rather than at each site, because a hint is what makes a 402 rotatable and
    // a non-CodeBuddy 402 must not become one by accident.
    const status = refusal.status;
    let hint: CodebuddyFailoverHint | undefined;
    if (route.providerName === CODEBUDDY_PROVIDER_ID) {
      hint = refusal instanceof Response
        ? await recoverCodebuddyFailoverHint(refusal, options.abortSignal)
        : refusal.message !== undefined
          ? codebuddyFailoverHintFromErrorPayload(refusal.message)
          : undefined;
    }
    // The failed account is captured HERE, not read again at the call site: the planner is
    // async and genericFailoverAccountId is mutable request state, so a re-read after this
    // await could cool an account other than the one whose refusal produced the plan.
    const failedAccountId = genericFailoverAccountId;
    if (!shouldAttemptGenericOAuthFailover(
      ...
      genericFailovers,
      hint,
    ) || !failedAccountId) return null;
   const modelId = normalizeGenericFailoverModelId(route.providerName, route.modelId);
   // A model-scoped cooldown needs a model to scope it to. Without one the pair cannot be
   // named, and cooling the whole account is the honest fallback rather than an unscoped
   // model key that would cool nothing.
   const scope = hint?.scope === "model" && modelId ? "model" : "account";
   return {
     failedAccountId,
      // A plain 429 keeps upstream's own target shape: the bare routed model id, which the
      // cooldown store reads as a quota FAMILY when the provider has one (Antigravity's
      // gem/cla) and as an exact model otherwise. Collapsing it into an explicit account
      // scope here would quietly widen every Antigravity cooldown to the whole account.
      target: hint ? {
        scope,
        modelId: scope === "model" ? modelId : route.modelId,
        cooldownUntilMs: hint.resetAtMs,
        quotaExhausted: true,
      } : route.modelId,
      // WorkBuddy 6004 names ONE account×model pair and must not move the operator's stored
      // selection; 14018 and an ordinary 429 cool the account and persist.
      persistActive: scope !== "model",
   };
 };
  ...
  const selectionIsCurrent = (binding: DispatchBinding | undefined): boolean => {
    ...
    // Model-scoped WorkBuddy 6004 rotation keeps the operator's active account on purpose and
    // rebinds only THIS request, so the stored selection still names the other account. Reading
    // that as "a concurrent manual switch happened" would refuse a dispatch the rotation just
    // authorized; the credential comparison above is what actually has to hold.
    if (
      ...
      if (route.providerName === "anthropic" && isAnthropicAccountPoolEnabled(config)) {
        ...
        const preferredAccountId = isGenericFailoverProvider(route.providerName, route.provider)
          ? preferredInitialAccount(
            ...
            Date.now(),
            route.modelId,
            route.providerName === CODEBUDDY_PROVIDER_ID
              ? parsed.options.promptCacheKey?.trim() || undefined
              : undefined,
          )
          : null;
        ...
        // Cooldown avoidance is not the proactive pool. With the knob off, preferredInitialAccount
        // only names a spare after this account×model (or the whole account) already 402/429'd --
        // it is the first attempt of the NEXT turn avoiding a replay of the last one's refusal.
        // Persisting that choice would silently move the operator's active account as a side
        // effect of a failure, so the rotation is applied to this request only.
        ...
        if (usedPreferredAccount && !persistPreferredAccount) {
          if (await applyFailoverSnapshot(resolved, parsed, false)) {
            ...
            // The unpersisted rebind refused (credential gone or needs reauth). Fall back to
            // admitting the resolved account through the ordinary path rather than dispatching
            // on a snapshot the store already rejected.
            usedPreferredAccount = false;
            resolved = await getValidAccessTokenSnapshot(route.providerName);
        ...
        // Account-scoped routing headers under the same contract as Kiro's above: an empty
        // object means a genuinely accountless request, which keeps the adapter's own
        // local/env behaviour rather than pinning headers from a previous account.
        if (route.providerName === CODEBUDDY_PROVIDER_ID) {

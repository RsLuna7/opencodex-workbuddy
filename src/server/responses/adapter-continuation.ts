  ...
  const fetchTerminalGuardContinuation = async function* (
    ...
    if (
       (response.status === 429 || response.status === 402)
        ...
        // Shared decision, per-site reservation: see planGenericOAuthRotation. This arm is
        // exactly the drift the comment above describes -- it has to admit the same refusals
        // the streaming loop does, including WorkBuddy's 402 + 6004/14018 hint.
        const genericRotation = await transportState.planGenericOAuthRotation(response);
        if (!genericRotation) break;
    ...
    const applied = await applyFailoverSnapshot(snapshot, nextParsed, genericRotation.persistActive);

  ...
  const {
    ...
    if (
     (upstreamResponse.status === 429 || upstreamResponse.status === 402)
      ...
      // Shared decision, per-site reservation: see planGenericOAuthRotation. The replay is
      // this hop's send, so the reservation below is what buys it; a refusal leaves the real
      // refusal -- body, Retry-After and any quota evidence -- exactly as upstream sent it.
      // A 429 is its own evidence; a 402 must carry WorkBuddy's 6004/14018 hint to qualify.
      const genericRotation = await transportState.planGenericOAuthRotation(upstreamResponse);
      // The roster bound above is one half of the bound; the request's shared budget is the
      // other. A refused hop leaves the real refusal exactly as upstream sent it.
      const hop = genericRotation
        ? reserveCredentialHop(
          "auth-recovery",
        ...
        : null;
      if (genericRotation && hop?.allowed) {
        const nextAccountId = rotateGenericOAuthAccountOn429(
          config, route.providerName, genericRotation.failedAccountId,
          upstreamResponse.headers.get("retry-after"),
          Date.now(),
          genericRotation.target,
        );
      ...
      if (genericRotation && snapshot
        && await applyFailoverSnapshot(snapshot, parsed, genericRotation.persistActive)) {
        ...
        hop.permit?.release();
      } else if (genericRotation) {
        // Rotation was available -- the roster decision above admitted it -- and the shared
        // request budget refused. Recorded so a one-send log is not read as "nothing was
        // eligible", which is the ambiguity this attribution exists to remove (#5044). A
        // null plan is NOT reported here: nothing was ever eligible to withhold.

  ...
  if (!upstreamResponse.ok) {
   ...
   recovery: for (;;) {
     ...
     if (
      ...
      // The decision is shared across every refusal site (see planGenericOAuthRotation); the
      // reservation and rebuild below stay per-site because each site settles its permit
      // differently. A 429 needs no hint; WorkBuddy's 6004/14018 arrive as 402 with one.
      // Re-planned every iteration: the retry can land on another refusal (a different
      // model's 6004, a whole-account 14018) whose scope and cooldown differ from the
      // first one's. A plan hoisted out of the loop would replay the first refusal's scope.
      for (
        let genericRotation = await transportState.planGenericOAuthRotation(upstreamResponse);
        genericRotation;
        genericRotation = await transportState.planGenericOAuthRotation(upstreamResponse)
      ) {
        ...
        const nextAccountId = rotateGenericOAuthAccountOn429(
          config,
          route.providerName,
          genericRotation.failedAccountId,
          upstreamResponse.headers.get("retry-after"),
          Date.now(),
        ...
        try {
          ...
          if (!await applyFailoverSnapshot(snapshot, parsed, genericRotation.persistActive)) {
            hop.permit?.release();
            break;
          }
          invalidateSameTargetRequest();

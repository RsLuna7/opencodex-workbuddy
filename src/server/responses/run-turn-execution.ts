...
export async function executeResponsesRunTurn(
  ...
  const { routedCompaction } = sidecarState;
    ...
    const rotateRunTurnAdapterOnPreflight429 = async (
      ...
      // Shared decision, per-site reservation: see planGenericOAuthRotation. A 429 needs no
      // hint; WorkBuddy's 6004/14018 arrive as a 402 carrying one, which is the whole reason
      // this predicate is not a bare status check.
      if (status !== 429 && status !== 402) return false;
      // A preflight adapter error carries no Response, so the refusal is described instead.
      // Only a 402 can be scoped by a hint; a 429 needs none (see planGenericOAuthRotation).
      const genericRotation = await transportState.planGenericOAuthRotation({
        status,
        message: error.message,
      });
      if (!genericRotation) return false;
      ...
      try {
        ...
        // A model-scoped cooldown rebinds this request only; the operator's active account
        // must survive a single model going quiet (WorkBuddy 6004).
    const admittedSnapshot = await applyFailoverSnapshot(snapshot, parsed, genericRotation.persistActive);
        if (!admittedSnapshot) {
          hop.permit?.release();
          return false;
        }

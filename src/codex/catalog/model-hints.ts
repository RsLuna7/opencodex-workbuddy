...
export function catalogHintsFromModelsApiItem(providerName: string, item: ProviderModelsApiItem): Partial<CatalogModel> {
  ...
  const contextWindow =
    positiveSafeInteger(
      ...
      // WorkBuddy / CodeBuddy CN publishes the window on the camelCase payload, not the
      // snake_case metadata every other catalog uses.
      item.maxInputTokens,
    );
  ...
  const inputModalities = modelInputModalities(item, capabilities)
    ?? (item.supportsImages === true ? ["text", "image"]
      : item.supportsImages === false ? ["text"]
        : undefined);
  // A display name is only useful when it actually differs from the id; echoing the id
  // would just pin the picker label to a slug the provider may rename.
